...
RSpec.describe EventsController, type: :controller do
  ...
  describe 'GET #index' do
    ...
    let!(:out_of_range_event_n1) { create(:event, start: '1980-01-01', end: '1999-01-01', user_id: current_user.id) }
    let!(:out_of_range_event_n2) { create(:event, start: '2025-01-01', end: '2030-01-01', user_id: current_user.id) }
    before do
      login(current_user)
      get :index, params: { start: '2007-01-01', end: '2022-01-01' }
    end

    it 'ensures user gets only his non-ocurring events' do
  ...
  describe 'GET #all' do
    ...
    let!(:recurring_events) { create_list(:event, 3, user_id: current_user.id, frequency: 'daily') }
    let!(:other_user_events) { create_list(:event, 3, user_id: user.id) }
    let!(:out_of_range_event_n1) { create(:event, start: '1980-01-01', end: '1999-01-01') }
    let!(:out_of_range_event_n2) { create(:event, start: '2025-01-01', end: '2030-01-01') }
    before do
      login(current_user)
      get :all, params: { start: '2007-01-01', end: '2022-01-01' }, format: :json
    end

    it 'ensures user gets all non-ocurring events' do
      expect(assigns(:events)).to match_array(events + other_user_events)
    end
  end
  ...
  describe 'POST #create' do
    ...
    context 'with invalid attributes' do
      it 'does not save the event' do
        expect { post :create, params: { event: attributes_for(:event, :invalid_event) } }.to_not change(Event, :count)
  ...
  describe 'PATCH #update' do
    context 'with user being author' do
      ...
      context 'with invalid attributes' do
        before { patch :update, params: { id: event.id, event: attributes_for(:event, :invalid_event) } }
